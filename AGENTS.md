...
- **UI email signup/login** mounts at `/api/auth/signup` and `/api/auth/login` (email router prefix is `/auth` under the `/api` api_router). Prefer **Demo Login** for quick access (`/api/auth/demo-login`). Production needs `REACT_APP_BACKEND_URL` pointing at a live FastAPI host — see `PRODUCTION_OPS.md`.
- **Feature routes mount via `routes/registry.py`** (not a hand-picked subset in `server.py`). Casino/table games (baccarat, blackjack, roulette, slots, founder engines, wave-2 casino, etc.) are soft-mounted there — check `GET /api/admin/route-registry` or `/docs`. HTTP test failures on these paths are usually auth (need demo-login), stake floors (`>= 50`), or stale URLs (e.g. `/api/baccarat/start` → `/api/baccarat/play`), not missing mounts.
