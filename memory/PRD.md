...
> **2026-05-09 (Latest) — Multi-Language Tour Video Infrastructure 🌍🗣️.** Founder ask post-deploy: "make the landing tour video change to whatever language a person speaks." Shipped the FRONTEND infrastructure now; the per-language MP3 generation is **PENDING because the Emergent LLM key budget is exhausted** (`budget_exceeded` 400 error on first TTS call).
>
> 1. **Frontend infra (LIVE on preview, deployed when next redeploy)** — `LandingTourVideo.tsx` now reads `/landing-tour-i18n.json` at runtime: tracks per language with `{label, native, rtl, audio, duration, cues[]}`. Auto-detects browser language via `navigator.languages` + falls back to `manifest.default = "en"`. Persists user override in `localStorage gv_tour_lang`. Re-mounts `<audio>` on language change (key={audioSrc}) so the new MP3 takes over cleanly. Captions get `dir="rtl"` for RTL languages.
> 2. **Globe-icon language picker** anchored top-right of the player (only visible after Play). Lists `native` name + 2-letter code for every language in the manifest. Closes on selection, switches audio + captions, scrubber resets, user re-clicks Play.
> 3. **Backend pipeline ready** — `/app/backend/scripts/generate_landing_tour_i18n.py` is built and ready to run. It (a) translates the English script + caption cues via Claude Haiku (preserves 30+ brand terms verbatim — VIBEZ, DSG, $VIBEZ, VibeRidez, Solana, etc.), (b) calls OpenAI TTS Onyx in each target language (Onyx adapts pronunciation natively, same voice across all locales), (c) captures actual MP3 duration to scale caption timings proportionally, (d) writes the manifest. Languages configured: en · es · fr · pt · zh · hi · ar · ja.
> 4. **Manifest seeded with English-only** so the player works perfectly out of the box. When the LLM key budget is topped up, running `python /app/backend/scripts/generate_landing_tour_i18n.py` generates the other 7 languages (~17 MB total, ~3 minutes wall time, ~$0.20 in API spend) and the picker auto-shows them on next page load.
> 5. **Regression Shield: 239/239 GREEN** (+1 lock `test_landing_tour_supports_multilanguage_manifest` enforces manifest schema, picker testids, language persistence, RTL caption support, brand-term preservation in the generation script).
>
> **ACTION REQUIRED:** add balance to your Emergent LLM Universal Key (Profile → Universal Key → Add Balance, or enable auto top-up). Then ssh into the preview shell and run: `cd /app/backend && python scripts/generate_landing_tour_i18n.py`. Then redeploy.
