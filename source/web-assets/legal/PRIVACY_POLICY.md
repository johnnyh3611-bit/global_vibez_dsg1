# PRIVACY POLICY
**Global Vibez Digital Social Gaming Platform**

*Last Updated: April 20, 2026*  
*Effective Date: April 20, 2026*

---

## 1. INTRODUCTION

Global Vibez DSG ("we", "us", "our") respects your privacy and is committed to protecting your personal data. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our Platform.

**This policy complies with:**
- General Data Protection Regulation (GDPR) - EU
- California Consumer Privacy Act (CCPA) - USA
- Personal Information Protection and Electronic Documents Act (PIPEDA) - Canada

---

## 2. INFORMATION WE COLLECT

### 2.1 Personal Information You Provide

**Account Registration:**
- Full name
- Email address
- Date of birth
- Phone number (optional)
- Username

**Identity Verification (for payouts):**
- Government-issued ID (passport, driver's license)
- Proof of address (utility bill, bank statement)
- Selfie for identity confirmation
- Social Security Number or Tax ID (required for payouts >$600 USD/year)

**Payment Information:**
- Credit/debit card details (stored securely by payment processor)
- Bank account information (for payouts)
- Payment transaction history

**User-Generated Content:**
- Profile pictures/avatars
- Chat messages
- Game preferences
- Support tickets

### 2.2 Information Collected Automatically

**Usage Data:**
- IP address
- Device type and operating system
- Browser type and version
- Pages visited and time spent
- Referring URLs
- Click patterns and interactions

**Gaming Activity:**
- Games played
- Bet amounts and frequency
- Win/loss records
- Session duration
- Timestamps of all activities

**Technical Data:**
- Cookies and similar tracking technologies
- Device identifiers
- Location data (country/region level)
- Error logs and crash reports

### 2.3 Information from Third Parties

**Authentication Providers:**
- Google OAuth: Name, email, profile picture
- Social media logins: Public profile information

**Payment Processors:**
- Helio / MoonPay (card checkout): Transaction data, payment method details
- Solana deposits: on-chain wallet address and transaction signature

**Fraud Prevention:**
- Third-party fraud detection services
- Credit bureau data (for identity verification)

---

## 3. HOW WE USE YOUR INFORMATION

### 3.1 Service Provision
- Create and manage your account
- Process transactions (deposits, withdrawals)
- Enable gameplay and platform features
- Provide customer support
- Send service-related communications

### 3.2 Security & Compliance
- Verify your identity (KYC/AML compliance)
- Detect and prevent fraud
- Monitor for suspicious activity
- Comply with legal obligations
- Enforce Terms of Service

### 3.3 Improvement & Analytics
- Analyze platform usage and performance
- Improve game design and user experience
- Conduct market research
- Develop new features
- Troubleshoot technical issues

### 3.4 Marketing (with consent)
- Send promotional emails and offers
- Personalize content and recommendations
- Display targeted advertisements
- Conduct surveys and feedback requests

**You can opt out of marketing at any time.**

---

## 4. LEGAL BASIS FOR PROCESSING (GDPR)

We process your data under the following legal grounds:

### 4.1 Contract Performance
- Account creation and management
- Processing payments
- Providing gaming services

### 4.2 Legal Obligation
- Identity verification (KYC/AML)
- Tax reporting (IRS Form 1099)
- Responding to legal requests

### 4.3 Legitimate Interest
- Fraud prevention
- Platform security
- Service improvement
- Analytics

### 4.4 Consent
- Marketing communications
- Optional features
- Cookies (non-essential)

**You may withdraw consent at any time.**

---

## 5. DATA SHARING & DISCLOSURE

### 5.1 We Share Data With:

**Service Providers:**
- Payment processors (Helio / MoonPay) and the Solana network
- Cloud hosting (AWS, Google Cloud)
- Analytics providers (Google Analytics)
- Customer support tools
- Email service providers
- Fraud detection services

**Legal Requirements:**
- Law enforcement (with valid legal request)
- Regulatory authorities
- Tax authorities (IRS, HMRC, etc.)
- Courts and legal counsel

**Business Transfers:**
- In case of merger, acquisition, or sale of assets
- Users will be notified of any change in data stewardship

### 5.2 We DO NOT:
- ❌ Sell your personal data to third parties
- ❌ Share data with advertisers without consent
- ❌ Use your data for unrelated purposes

---

## 6. DATA RETENTION

### 6.1 Active Accounts
- Personal data: Retained while account is active
- Transaction records: 7 years (legal requirement)
- Chat logs: 90 days
- Game history: 2 years

### 6.2 Closed Accounts
- Identity documents: 7 years (AML compliance)
- Financial records: 7 years (tax compliance)
- Other data: Deleted within 30 days of closure

### 6.3 Legal Holds
- Data subject to legal proceedings is retained until resolution

---

## 7. YOUR PRIVACY RIGHTS

### 7.1 GDPR Rights (EU/EEA)

**Right to Access:**
- Request a copy of your personal data
- Provided in machine-readable format

**Right to Rectification:**
- Correct inaccurate or incomplete data

**Right to Erasure ("Right to be Forgotten"):**
- Request deletion of your data
- Subject to legal retention requirements

**Right to Restrict Processing:**
- Limit how we use your data

**Right to Data Portability:**
- Receive your data in structured format
- Transfer to another service provider

**Right to Object:**
- Object to processing based on legitimate interest
- Object to marketing communications

**Right to Withdraw Consent:**
- Withdraw consent at any time (does not affect previous processing)

**Right to Lodge a Complaint:**
- File complaint with supervisory authority

### 7.2 CCPA Rights (California)

**Right to Know:**
- Categories of data collected
- Sources of data
- Purposes for collection
- Third parties we share with

**Right to Delete:**
- Request deletion of personal data

**Right to Opt-Out:**
- Opt out of sale of personal data (we don't sell data)

**Right to Non-Discrimination:**
- Equal service regardless of privacy rights exercise

### 7.3 How to Exercise Rights

**Email:** privacy@globalvibez.com  
**Subject:** Privacy Rights Request  
**Include:** Full name, email, account username, specific request

**Response Time:** 30 days (may extend to 60 days for complex requests)

---

## 8. DATA SECURITY

### 8.1 Security Measures

**Technical:**
- 256-bit SSL/TLS encryption
- Secure HTTPS connections
- Encrypted databases
- Regular security audits
- Intrusion detection systems

**Organizational:**
- Access controls and authentication
- Employee training on data protection
- Regular security assessments
- Incident response plan

**Payment Security:**
- PCI DSS Level 1 compliant processors
- Tokenization of payment data
- No storage of full card numbers

### 8.2 Your Responsibilities
- Use strong, unique passwords
- Enable two-factor authentication (2FA)
- Keep login credentials confidential
- Report suspected security breaches

**We cannot guarantee absolute security. Report breaches to: security@globalvibez.com**

---

## 9. COOKIES & TRACKING

### 9.1 Cookie Types

**Essential Cookies:**
- Authentication and session management
- Security and fraud prevention
- Platform functionality

**Analytics Cookies:**
- Google Analytics
- Usage patterns and statistics
- Performance monitoring

**Marketing Cookies:**
- Personalized advertisements
- Campaign tracking
- Social media integration

### 9.2 Cookie Control

**You can:**
- Accept or reject cookies via our cookie banner
- Adjust browser settings to block cookies
- Delete existing cookies

**Note:** Blocking essential cookies may impair platform functionality.

**Cookie Policy:** [Link to detailed cookie policy]

---

## 10. INTERNATIONAL DATA TRANSFERS

### 10.1 Data Location
- Primary servers: United States (AWS US-East)
- Backup servers: European Union (AWS EU-West)

### 10.2 Transfer Safeguards
- Standard Contractual Clauses (SCCs) - GDPR
- Privacy Shield Framework (where applicable)
- Adequacy decisions by EU Commission

**EEA users:** Your data may be transferred outside the EEA with appropriate safeguards.

---

## 11. CHILDREN'S PRIVACY

**Platform Restriction:**
- Platform is **NOT** intended for users under 18
- We do not knowingly collect data from minors
- Age verification required upon registration

**If you believe a minor has provided data:**
- Email: privacy@globalvibez.com
- We will delete the data immediately

---

## 12. CALIFORNIA "DO NOT TRACK" DISCLOSURE

- We do not currently respond to Do Not Track (DNT) signals
- Third-party tracking may occur (Google Analytics, etc.)
- You can disable tracking via browser settings or extensions

---

## 13. CHANGES TO THIS POLICY

- We may update this Privacy Policy periodically
- Material changes: 30-day advance notice via email/platform notification
- Minor changes: Posted immediately with updated "Last Updated" date
- Continued use constitutes acceptance

**Review policy regularly:** [Link to this policy]

---

## 14. CONTACT INFORMATION

### 14.1 Privacy Inquiries

**Data Protection Officer (DPO):**
- Email: dpo@globalvibez.com
- Mail: [Your Business Address]

**General Privacy:**
- Email: privacy@globalvibez.com
- Support: support@globalvibez.com

### 14.2 Supervisory Authority (GDPR)

**EU Users:**
- Contact your local data protection authority
- List: https://edpb.europa.eu/about-edpb/board/members_en

**UK Users:**
- Information Commissioner's Office (ICO)
- Website: https://ico.org.uk/

---

## 15. RESPONSIBLE GAMING & DATA

### 15.1 Self-Exclusion Data
- Self-exclusion requests are permanent records
- Cannot be deleted during exclusion period
- Retained for compliance purposes

### 15.2 Problem Gambling Support
- Your data may be shared with problem gambling organizations (with consent)
- Used to enforce deposit limits and self-exclusion

---

## 16. AUTOMATED DECISION-MAKING

### 16.1 Profiling
- We use algorithms to detect fraud and suspicious activity
- Automated systems may flag accounts for review
- **You have the right to request human review of automated decisions**

### 16.2 Anti-Money Laundering (AML)
- Automated transaction monitoring
- Risk scoring for compliance
- Manual review for high-risk transactions

---

**BY USING THE PLATFORM, YOU ACKNOWLEDGE THAT YOU HAVE READ AND UNDERSTOOD THIS PRIVACY POLICY.**

---

*This is a template. Consult with a licensed attorney and data protection officer to ensure full compliance with applicable privacy laws in your jurisdiction.*
