...
export default function BlackjackUniversal() {
  ...
  return (
    <GameRoomLayout
      testId="bj-universal-room"
      title="Blackjack"
      subtitle="Universal · Multi-Seat"
      backTo="/games"
      nativeTable
      hudExtra={
        <div
          className="text-xs bg-black/40 backdrop-blur rounded-full px-3 py-1.5 border border-cyan-500/30"
          data-testid="bj-shoe-count"
        >
      ...
      table={
        <div className="w-full max-w-5xl px-2">
          ...
          <div className="text-center mb-4" data-testid="bj-dealer-area">
            <div className="inline-block bg-black/60 backdrop-blur border border-amber-400/40 rounded-xl px-6 py-3">
              <div className="text-xs uppercase text-amber-300 mb-2">
                Dealer NOVA {state.phase !== 'betting' && `• ${state.dealer.value}`}
              </div>
              ...
              {state.dealer.is_blackjack && (
                <div className="text-rose-400 font-bold mt-2">BLACKJACK!</div>
              )}
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {seat.bet > 0 && (
                      <span className="bg-amber-500/20 border border-amber-400 rounded-full px-2 py-0.5 text-amber-300 text-xs">
                    ...
                    <div className="mt-2">
                      <div
                        className={`text-lg font-bold ${
                          seat.value > 21 ? 'text-rose-400' : 'text-cyan-300'
                        }`}
                      >
                        {seat.value}
                        {seat.value > 21 ? ' BUST' : ''}
          ...
          {/* Hidden back control for existing tests */}
          <button
            type="button"
            className="sr-only"
      ...
      actions={
        <>
          {state.phase === 'betting' && (
            <div
              className="bg-black/60 backdrop-blur border border-cyan-400/40 rounded-2xl p-4 max-w-md w-full"
              data-testid="bj-betting-ui"
            >
              <h3 className="text-center text-sm font-bold mb-3">Select Bet (all seats match)</h3>
